...
int main()
{
	...
	while(scanf("%li",&size))
	{
		if(size==0)
			break;			
		long int arr[size+1],arr_IP[size+1];
		for(long int i=1;i<=size;i++)		// Taking input for this problem. 
			scanf("%li",&arr[i]);
			
		long int temp=0,count=0;
		
		for(long int i=1;i<=size;i++)		// This is the main logic for this problem. 
			arr_IP[arr[i]]=i;
		
		for(long int i=1;i<=size;i++)		// Here we verify, whether our main array is equal with Inversed array or not. 
		{
//			printf("%li %li \n",arr[i],arr_IP[i]);
			if(arr[i]==arr_IP[i])
				count++;					// and we count for each equal inputs. 
		}
		if(count==size)
			printf("ambiguous\n");
		else
...
So the natural way to represent a permutation is to list the integers in this order. With n = 5, a permutation might look like 2, 3, 4, 5, 1. 
